...
import { z } from "zod";
import { BookingDetails } from "../types/interface";
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await axios.post(
                ...
                {
                    ...formData,
                    // the rest of the fields are handled by backend
                },
                {
                    headers: {
                        "X-API-KEY": apiKey,
            ...
            console.log("Checking your book", response.data.data)
            ...
            >
                ...
                >
                    <div className="flex flex-col w-full gap-2">
                        ...
                        <div className="flex items-center rounded-full border border-[#000929] px-5 gap-[10px] transition-all duration-300 focus-within:ring-2 focus-within:ring-[#074799]">
                            <img
                        ...
                        {formErrors.find((error) => error.path.includes("booking_trx_id")) && (
                            <p className="text-red-500 font-semibold">Booking ID is required</p>
                        )}
                    </div>
                    <div className="flex flex-col w-full gap-2">
                        ...
                        {formErrors.find((error) => error.path.includes("phone_number")) && (
                            <p className="text-red-500 font-semibold">Phone number is required</p>
                        )}
                    </div>
                    <button
                        disabled={isLoading}
                        type="submit"
                        className="flex items-center justify-center rounded-full p-[12px_30px] gap-3 bg-[#074799] font-bold text-[#F7F7FD]"
                    >
                        ...
                        <div className="flex flex-col h-fit shrink-0 rounded-[20px] border border-[#E0DEF7] p-[30px] gap-[30px] bg-white">
                            ...
                            <div className="flex flex-col gap-5">
                                {!bookingDetails.is_paid ? (
                                    ...
                                    <div className="flex items-center justify-between">
                                        <p className="font-semibold">Status Pembayaran</p>
                                        <p className="rounded-full w-fit p-[6px_16px] bg-[#074799] font-bold text-sm leading-[21px] text-[#F7F7FD]">
                                            SUCCESS
                                        </p>
                                    </div>
                                )}
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <p className="font-bold text-[22px] leading-[33px] text-[#074799]">
